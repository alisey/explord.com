...
function Brain(world, predictOn) {
    ...
    function plannedMove() {
        var move = plan.shift();
        if (!move) {plan = [] }
        if (move && Coords.equal(world.appleCoords(), Coords.add(world.headCoords(), move))) {
            console.log('Going to eat apple now while following plan ' + plan.type + ' at snake length ' + world.snake.length);
        }
