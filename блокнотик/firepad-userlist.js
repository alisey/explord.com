var FirepadUserList = (function() {
  function FirepadUserList(ref, place, userId, displayName, color) {
  ...
  FirepadUserList.prototype.makeUserEntryForSelf_ = function() {
    ...
    var nameDiv = elt('div', this.displayName || 'You', {
      'class': 'firepad-userlist-name'
    });
    emojify(nameDiv);
    this.firebaseOn_(myUserRef.child('name'), 'value', function(nameSnapshot) {
      var name = nameSnapshot.val();
      if (!name) name = this.displayName; // @@@
      this.displayName = name;
      nameDiv.innerHTML = '';
      nameDiv.appendChild(document.createTextNode(name || 'You'));
      emojify(nameDiv);
    }.bind(this));

    on(nameDiv, 'click', function() {
      var name = prompt("What's your name, darling?", this.displayName);
      if (name) {
        setName(name);
      }
    }.bind(this));

    on(colorDiv, 'click', function(event) {
      colorPicker(event.pageX - 120, event.pageY, setColor);
      // var color = prompt("What color do you fancy?", this.color || '');
  ...
  FirepadUserList.prototype.setName = function(newName) {
    var myUserRef = this.ref_.child(this.userId_);
    myUserRef.child('name').onDisconnect().remove();
    myUserRef.child('name').set(newName);
  };


  FirepadUserList.prototype.makeUserEntriesForOthers_ = function() {
    var self = this;
    var userList = elt('div', null, {'class': 'firepad-userlist-users-other'});
    var userId2Element = { };
    ...
    function updateChild(userSnapshot, prevChildName) {
      ...
      if (typeof name !== 'string') { name = 'Stranger'; }
      ...
      var colorDiv = elt('div', null, {'class': 'firepad-userlist-color-indicator'});
      colorDiv.style.backgroundColor = color;
      var nameDiv = elt('div', name || 'Stranger', {
        'class': 'firepad-userlist-name'
      });
      emojify(nameDiv);
      userDiv = elt('div', [colorDiv, nameDiv], {
...
function colorPicker(x, y, onselect) {
    var colors = [
    ...
    for (var i = 0; i < colors.length; i++) {
        ...
        swatch.setAttribute('data-color', colors[i]);
    ...
    function destroy() {
        ...
        document.removeEventListener('keydown', destroy);
